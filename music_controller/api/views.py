...
class GetRoom(APIView):
    ...
    def get(self, request, format=None):
        ...
        if code is not None:
            ...
            if len(room) > 0:
                # only one room so can access at room[0]
                data = RoomSerializer(room[0]).data
                # the host's session key is stored in room data
...
class JoinRoom(APIView):
    ...
    def post(self, request, format=None):
        ...
        # if code field is present, query room from database
        if code is not None:
            room_result = Room.objects.filter(code=code)
            if len(room_result) > 0:
                room = room_result[0]
                # creating a temporary storage object
...
class CreateRoomView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            # If we are updating existing room
            if queryset.exists():
                room = queryset[0]
...
class UpdateRoom(APIView):
    ...
    def patch(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            room = queryset[0]
            # check user trying to update room is host
